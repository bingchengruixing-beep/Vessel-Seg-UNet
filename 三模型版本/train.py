...
def build_optimizer(model, config: dict) -> torch.optim.Optimizer:
    """根据配置构建优化器（支持差分学习率：编码器低 LR）"""
    train_cfg = config['training']
    lr = train_cfg['learning_rate']
    wd = train_cfg.get('weight_decay', 1e-4)
    name = train_cfg.get('optimizer', 'adamw').lower()
    encoder_lr_scale = train_cfg.get('encoder_lr_scale', 1.0)

    # 差分学习率：编码器用更低 LR（迁移学习精细微调）
    if encoder_lr_scale != 1.0 and hasattr(model, 'get_param_groups'):
        params = model.get_param_groups(lr, encoder_lr_scale)
    else:
