...
class UNetResNet(nn.Module):
    ...
    def __init__(
        ...
        self.dec4 = DecoderBlock(512, 256, 256)  # layer4 + layer3 → 256
        self.dec3 = DecoderBlock(256, 128, 128)  # + layer2        → 128
        self.dec2 = DecoderBlock(128, 64, 64)    # + layer1        → 64
        self.dec1 = DecoderBlock(64, 64, 32)     # + stem          → 32
    ...
    def get_param_groups(self, base_lr: float, encoder_lr_scale: float):
        """
        返回差分学习率的参数分组：编码器（预训练）用更低学习率，解码器用基础学习率。

        Args:
            base_lr: 基础学习率（解码器）
            encoder_lr_scale: 编码器学习率缩放系数（如 0.1 → 编码器 lr = base_lr * 0.1）

        Returns:
            参数组列表，供 torch.optim 使用
        """
        encoder_prefix = ('conv1', 'bn1', 'layer1', 'layer2', 'layer3', 'layer4')
